...
public class FieldFactory {
    ...
    private static final String TAG = FieldFactory.class.getSimpleName();
    ...
    public FieldFactory() {
        ...
        try {
            ...
            inputStreamReader = new InputStreamReader(am.open("ISO8583Def.xml")
                    , Charset.forName("utf-8"));
            predefine(inputStreamReader);

            Log.d(TAG, "FieldFactory complete,define size={}=" + define.size());
        ...
        switch (field.getType()) {
            case BCD:
                if ((length + 1) / 2 != data.length)
                    throw new RuntimeException("Set value's length (" + data.length + ") doesn't equal with predefining length (" + (length + 1) / 2 + ")");
                break;
            case ASCII:
                if (length != data.length)
                    throw new RuntimeException("Set value's length (" + data.length + ") doesn't equal with predefining length (" + length + ")");
                break;
            case BINARY:
                if ((length / 8) != data.length)
                    throw new RuntimeException("Set value's length (" + data.length + ") doesn't equal with predefining length (" + (length / 8) + ")");
                break;
        }
